...
test.describe('Settings', () => {
  ...
  test('export triggers download on web', async ({
    settingsPage,
    page,
  }) => {
    await settingsPage.expandExportData();
    ...
    expect(download.suggestedFilename()).toMatch(/^debtinator-export-\d{4}-\d{2}-\d{2}\.xlsx$/);
  });

  test('Help section has Features Guide link', async ({ settingsPage }) => {
    await settingsPage.assertHelpSectionVisible();
    await expect(settingsPage.getByText('Learn how to use all app features')).toBeVisible();
  });
