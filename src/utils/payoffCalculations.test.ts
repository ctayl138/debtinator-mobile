...
describe('payoffCalculations', () => {
  describe('getDebtSummary', () => {
    ...
    it('returns weightedInterestRate 0 when totalBalance is 0 (no debts with balance)', () => {
      const debts = [
  ...
  describe('calculatePayoffSchedule', () => {
    ...
    it('caps at max months when monthly payment is 0 (cannot pay off)', () => {
      ...
      const result = calculatePayoffSchedule(plan);
      // With 0 payment, debt can never be paid off - hits safety cap
    ...
    it('snowball: pays smallest balance first', () => {
      ...
      // First month: small debt gets minimum 25 + extra from 200 - 100 - 25 = 75 extra to small
      // So small gets 25+75=100, big gets 100. Small balance 500, after interest ~508, payment 100 -> 408, etc.
      const firstMonth = result.steps[0];
      expect(Array.isArray(firstMonth)).toBe(true);
    ...
    it('single debt paid off in a few months', () => {
      ...
      expect(result.totalPayments).toBeGreaterThanOrEqual(300);
    });
    ...
    it('custom: includes debts not in customOrder', () => {
      ...
      // Only include 'a' in customOrder, 'b' and 'c' should be added at the end
      const plan: PayoffPlan = {
        method: 'custom',
        monthlyPayment: 200,
        debts,
    ...
    it('handles unknown method by using default order', () => {
      ...
      const result = calculatePayoffSchedule(plan);
      expect(result.totalMonths).toBeGreaterThan(0);
      // Should process in original order (neither sorted by balance nor interest)
    ...
    it('adds new step for priority debt when no existing step', () => {
      // Create a scenario where minimum payment is 0 so no step is created for priority debt
      // during minimum payment phase, but extra payment goes to it
      const debts = [
      ...
      const result = calculatePayoffSchedule(plan);
      expect(result.totalMonths).toBeGreaterThan(0);
      // Priority debt should get all the payment since minimum is 0
      const firstMonth = result.steps[0];
